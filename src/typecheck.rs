...
impl<'sess> Visitor for TypeChecker<'sess> {
    ...
    fn visit_function(&mut self, refid: NodeID, vis: Visibility, ident: &Option<Ident>, args: &Vec<Argument>, rettype: &Option<Type>, body: &Expr, abi: ABI) -> Result<Self::Return, Error> {
        let scope = self.stack.get_scope();
        ...
        for ref arg in args.iter() {
            ...
            let mut atype = self.session.get_type(arg_defid).unwrap_or_else(|| fscope.new_typevar(self.session, false));

            // TODO default args aren't used yet
            //let vtype = arg.default.clone().map(|ref vexpr| self.visit_node_expected(vexpr, ttype.clone()));
            //atype = expect_type(self.session, fscope.clone(), atype, vtype, Check::Def)?;
            ...
            if &arg.ident.name[..] == "self" {
                ...
                println!(">>> {:?} {:?} self {:?}", arg.id, arg_defid, atype);
            }
        ...
        let rettype = self.with_scope(fscope.clone(), |visitor| {
            expect_type(visitor.session, fscope.clone(), Some(rtype.clone()), Some(visitor.visit_node_expected(body, Some(rtype.clone()))), Check::Def)
        })?;
        ...
        self.session.update_type(scope.clone(), refid, nftype.clone())?;
        Ok(nftype)
    }
    ...
    fn visit_invoke(&mut self, refid: NodeID, fexpr: &Expr, args: &Vec<Expr>, fid: NodeID) -> Result<Self::Return, Error> {
        ...
        let dtype = self.session_find_variant(tscope.clone(), refid, fexpr, &atypes)?;
        ...
        let ftype = match etype {
            Type::Function(args, rettype, abi) => {
                //let ftype = expect_type(self.session, tscope.clone(), Some(etype.clone()), Some(Type::Function(r(atypes), r(self.session.get_type(refid).unwrap_or_else(|| tscope.new_typevar(self.session, false))), abi)), Check::Update)?;
            ...
            Type::Variable(_, vid, _) => {
                let ftype = Type::Function(r(atypes), r(self.session.get_type(refid).unwrap_or_else(|| tscope.new_typevar(self.session, false))), ABI::Unknown);
        ...
        //self.session.update_type(scope.clone(), refid, ftype.clone())?;
        let rtype = ftype.get_rettype()?.clone();
        self.session.set_type(fid, ftype);
        self.session.update_type(scope.clone(), refid, rtype.clone())?;
        Ok(rtype)
    }
    ...
    fn visit_side_effect(&mut self, _id: NodeID, _op: &Ident, args: &Vec<Expr>) -> Result<Self::Return, Error> {
        ...
        for ref expr in args {
            ltype = Some(expect_type(self.session, scope.clone(), ltype.clone(), Some(self.visit_node_expected(expr, ltype.clone())), Check::List)?);
        }
        Ok(ltype.unwrap())
    }
    ...
    fn visit_definition(&mut self, refid: NodeID, _mutable: Mutability, _ident: &Ident, _ttype: &Option<Type>, expr: &Expr) -> Result<Self::Return, Error> {
        ...
        let btype = expect_type(self.session, scope.clone(), dtype.clone(), Some(self.visit_node_expected(expr, dtype)), Check::Def)?;
        self.session.update_type(scope.clone(), defid, btype.clone())?;
        self.session.update_type(scope.clone(), refid, btype.clone())?;
        Ok(btype)
    }

    fn visit_declare(&mut self, refid: NodeID, _vis: Visibility, _ident: &Ident, _ttype: &Type) -> Result<Self::Return, Error> {
        let scope = self.stack.get_scope();
    ...
    fn visit_identifier(&mut self, refid: NodeID, ident: &Ident) -> Result<Self::Return, Error> {
        ...
        if let Ok(ttype) = self.session.get_type_from_ref(refid) {
            debug!("&&&&& {:?} {:?}", self.session.get_def(refid), ttype);
            Ok(ttype)
        } else {
            debug!("&&&&& {:?} {:?} NO TYPE {:?}", refid, self.session.get_ref(refid), &ident.name);
    ...
    fn visit_match(&mut self, _id: NodeID, cond: &Expr, cases: &Vec<MatchCase>) -> Result<Self::Return, Error> {
        ...
        for ref case in cases {
            ...
            self.with_scope(lscope.clone(), |visitor| {
                ctype = expect_type(visitor.session, lscope.clone(), Some(ctype.clone()), Some(visitor.visit_pattern_expected(&case.pat, Some(ctype.clone()))?), Check::List)?;
                rtype = Some(expect_type(visitor.session, lscope.clone(), rtype.clone(), Some(visitor.visit_node_expected(&case.body, rtype.clone())), Check::List)?);
    ...
    fn visit_raise(&mut self, _id: NodeID, expr: &Expr) -> Result<Self::Return, Error> {
        ...
        expect_type(self.session, scope.clone(), Some(extype.clone()), Some(self.visit_node_expected(expr, Some(extype))), Check::Def)?;
        scope.make_obj(self.session, String::from("()"), vec!())
    }
    ...
    fn visit_nil(&mut self, refid: NodeID) -> Result<Self::Return, Error> {
        let scope = self.stack.get_scope();
        let ttype = self.session.get_type(refid).unwrap_or_else(|| scope.new_typevar(self.session, false));
    ...
    fn visit_ref(&mut self, refid: NodeID, expr: &Expr) -> Result<Self::Return, Error> {
        ...
        let rtype = expect_type(self.session, scope.clone(), self.session.get_type(refid), Some(rtype), Check::Def)?;
    ...
    fn visit_deref(&mut self, refid: NodeID, expr: &Expr) -> Result<Self::Return, Error> {
        ...
        match self.visit_node_expected(expr, None) {
            Type::Ref(etype) => {
                let etype = expect_type(self.session, scope.clone(), self.session.get_type(refid), Some(*etype), Check::Def)?;
                self.session.set_type(refid, etype.clone());
                Ok(etype)
            },
            Type::Variable(_, vid, _) => {
                let etype = expect_type(self.session, scope.clone(), self.session.get_type(refid), None, Check::Def)?;
    ...
    fn visit_tuple(&mut self, refid: NodeID, items: &Vec<Expr>) -> Result<Self::Return, Error> {
        ...
        for (ref expr, etype) in items.iter().zip(etypes.iter()) {
            types.push(expect_type(self.session, scope.clone(), etype.clone(), Some(self.visit_node_expected(expr, etype.clone())), Check::List)?);
        }
    ...
    fn visit_class(&mut self, refid: NodeID, classspec: &ClassSpec, _parentspec: &Option<ClassSpec>, body: &Vec<Expr>) -> Result<Self::Return, Error> {
        let scope = self.stack.get_scope();
        let defid = self.session.get_ref(refid)?;
    ...
    fn visit_accessor(&mut self, node: &Expr, _left: &Expr, _right: &Ident, _oid: NodeID) -> Result<Self::Return, Error> {
        ...
        Ok(self.get_type_or_new_typevar(scope, defid, self.session.get_type(refid)))
    }
    ...
    fn visit_pattern_wild(&mut self, id: NodeID) -> Result<Self::Return, Error> {
        let scope = self.stack.get_scope();
        Ok(self.session.get_type(id).unwrap_or_else(|| scope.new_typevar(self.session, false)))
    }
    ...
    fn visit_pattern_resolve(&mut self, id: NodeID, left: &Pattern, field: &Ident, oid: NodeID) -> Result<Self::Return, Error> {
        let scope = self.stack.get_scope();
        ...
        expect_type(self.session, scope, Some(ttype), self.session.get_type(id), Check::Def)
    }
    ...
    fn visit_pattern_enum_args(&mut self, id: NodeID, left: &Pattern, args: &Vec<Pattern>) -> Result<Self::Return, Error> {
        ...
        match enumdef.get_variant_type_by_id(variant_id) {
            ...
            Some(ttype) => {
                ...
                let ctype = expect_type(self.session, tscope.clone(), Some(vtype), self.session.get_type(id), Check::Def)?;
...
impl<'sess> TypeChecker<'sess> {
    ...
    pub fn get_access_ids(&mut self, scope: ScopeRef, node: &Expr) -> Result<Option<(NodeID, NodeID)>, Error> {
        match &node.kind {
    ...
    pub fn session_find_variant(&mut self, scope: ScopeRef, invid: NodeID, fexpr: &Expr, argtypes: &Type) -> Result<Type, Error> {
